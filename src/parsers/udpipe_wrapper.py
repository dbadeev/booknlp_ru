...
class UDPipeParser:
    ...
    def parse_text(
        ...
        """
        Парсит текст с разбиением на чанки и возвращает список предложений с CoNLL-U полями.
        ...
        Args:
            ...
            tokenizer:          "native" | "razdel".
                                - ``"native"`` — токенизация выполняется самим UDPipe;
                                    поле ``misc`` содержит ``SpaceAfter``/``SpacesAfter``,
                                    пригодные для реконструкции оригинального текста.
                                - ``"razdel"`` — предтокенизация через razdel, затем
                                    UDPipe работает в horizontal-формате; поле ``misc``
                                    **всегда пустое** (``"_"`` / ``{}``), поскольку UDPipe
                                    не имеет информации об оригинальных пробелах.
                                    Для реконструкции текста используйте ``sent_text``
                                    и ``sent_start`` из возвращаемой структуры.
            output_format:      "dict" | "native".
            sentence_batch_size: количество предложений в одном Modal-чанке.

            Args:
        tokenizer: Режим токенизации.
            - ``"native"`` — токенизация выполняется самим UDPipe;
              поле ``misc`` содержит ``SpaceAfter``/``SpacesAfter``,
              пригодные для реконструкции оригинального текста.
            - ``"razdel"`` — предтокенизация через razdel, затем
    ...
    def parse_batch(
        ...
        Замечание: В режиме ``tokenizer="razdel"`` поле ``misc`` у всех токенов
        пустое — см. предупреждение в `parse_text`.
        """
        if not texts:
            return []
